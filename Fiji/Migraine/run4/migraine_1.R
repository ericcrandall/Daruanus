...
GP$FONKgNames <- c("twoNmu","latt2Ns2","g")
GP$FONKgScale <- c("Canonical","logscale","Canonical")
names(GP$FONKgScale) <- GP$FONKgNames
GP$samplingSpace<-c("twoNmu","latt2Ns2","g") ## ONLY for next points
GP$samplingScale<-c("Canonical","logscale","Canonical") ## ONLY for next points
...
GP$plotRange<-list()
GP$graphicPars<-list()
GP$plotOptions<-c("NoSurfaces")
GP$oneDimCIvars<-c("twoNmu","twoNm","Nb","condS2") ## (an empty c() value will be meaningful for nextBounts)
GP$spec1DProfiles<-c("twoNmu","twoNm","Nb","condS2","g")
GP$spec2DProfiles<-c()
GP$S2factor<-1 ##  other values add more confusion that anything else
GP$D2IBDbool<-FALSE;GP$D1IBDbool<-TRUE 
GP$Nbfactor<-35.0933 ##  correction from 2NmS2 to 2Ds2=Nb
...
write("Data file: ../../Daruanus_Fiji.txt",file=GP$cleanResu)
