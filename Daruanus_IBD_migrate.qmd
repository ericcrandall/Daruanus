...
Let's	explore	Ne	and	Fis	when	lumping	populations...		Lump populations that are 10, 20, 40, 100 and 200 km apart.
...
I	also	want	to	measure	gene	flow	between	archipelagos	with	migrate-n	and	to	show	that	this	is	doable	for	the	proposal.
...
Now	I'm	going	to	add	divergence	with	gene	flow	(going	from	West	to	East).	Also	change	the	migration	parameters	to	uniform.
...
```{bash}
#| eval: false
scp	-r	./Models2	naut:./Daruanus2/rep1

for	a	in	$(seq	2	5);	do	cp	-r	rep1	rep$a;	done
```
		...
		date	>	date.txt
			for	m	in	*/
				...
				cd	$m
						...
						echo	$m
						date
						mpirun	-np	32	~/eric_data/migrate-4.4.4/src/migrate-n-mpi	parmfile
						sleep	1
				cd	..
					done
		cd	..
...
rsync	-av	-e	ssh	--exclude='bayes*'	--exclude="*.pdf"	--exclude="*.mig"	ecrandall@nautilus.psu.edu:~/daruanus/	output
...
#|	message:	false
#|	results:	hide
runDir	<-	"daruanus/migrate/output"

likelist	<-	list()
for(r	in	1:10){
		rep	=	paste0("rep",r)
		print(rep)
		likelist[[rep]]	<-	harvest.model.likelihoods(workingDir=
																																																	file.path(runDir,rep))
}
...
```{r}
theta_prior_params	<-	c(0,0.1,1.0)
nm_prior_params	<-	c(0,300)
split_prior_params	<-	c(0,0.01,1)
...
```{r}
winningModelDir	<-	"daruanus/migrate/output/rep1/EtoW_DivMig"
...
popkey	<-	read_csv("daruanus/popkey.csv")
...
ggplot(mmu_long,	aes(x	=	x,	y	=	Density,	
								...
								scale_color_discrete(type=nm.palette(12),	
								                     labels	=	parameter_key) +
								xlim(7e6,7.5e6)
#	get	stats	for	each	summed-over-loci	posterior

mmu_stats	<-	parameter_key2	%>%	map_dfr(.f	=	~	posterior_stats(df	=	mmu,																																								parameter	=	.x),
																												.id	=	"parameter")
mmu_stats
```
...
parameters	<-	names(posterior)	%>% str_subset(parameter_type)
...
nm_stats	<-	parameter_key2	%>%	map_dfr(.f	=	~	posterior_stats(df	=	Nm,																																										parameter	=	.x),
																													.id	=	"parameter")
nm_stats
```
...
```{bash}
#| eval: false
(base)	eric@Saraswati	~/github/IBD_Kernels/Daruanus/treemix$	treemix	-i	Daruanus_treemix.txt.gz	-o	PNG_0m	-micro	-root	PNG	-global																																																																																																																																																																					✖	✹	✭main	
...
There	are	never	more	than	two	edges	added.	One	from	Fiji	to	Societies,	and	the	other	from	New	Caledonia	to	Tuamotus...

```{r}
source("Daruanus/treemix/plotting_funcs.R")
setwd("Daruanus/treemix/5pop_explore")
plot_tree("PNG_0m")
...
for	m	in	{1..10};	do
								...
								s=$RANDOM
								treemix	-i	../Daruanus_treemix.txt.gz	-o	PNG.${m}	-global	-m	${m}	-seed	${s}	-micro
done

plot_tree("PNG.1")
```

The	log	likelihood	reaches	a	plateau	of	-21.9889	at	6	migration	events

##	Orientagraph

Installed	according	to	instructions	given	[here](https://github.com/sriramlab/OrientAGraph)

```{bash}
#| eval: false
					

orientagraph	-i	../Daruanus_treemix.txt.gz	-o	PNG_0m_mlno	-micro	-root	PNG	-global	-m	0	-mlno

orientagraph	-i	../Daruanus_treemix.txt.gz	-o	PNG_1m_mlno	-micro	-root	PNG	-global	-m	1	-mlno	1

orientagraph	-i	../Daruanus_treemix.txt.gz	-o	PNG_2m_mlno	-micro	-root	PNG	-global	-m	2	-mlno	1,2

orientagraph	-i	../Daruanus_treemix.txt.gz	-o	PNG_3m_mlno	-micro	-root	PNG	-global	-m	3	-mlno	1,2,3	

orientagraph	-i	../Daruanus_treemix.txt.gz	-o	PNG_4m_mlno	-micro	-root	PNG	-global	-m	4	-mlno	1,2,3,4	

orientagraph	-i	../Daruanus_treemix.txt.gz	-o	PNG_5m_mlno	-micro	-root	PNG	-global	-m	5	-mlno	1,3,5	


```

```{r}
setwd("orientagraph_5pop/")
plot_tree("Tua_2m")
plot_tree("PNG_6m_mlno135")
plot_resid("PNG_6m_mlno135","../5poporder.txt")
```
